...
/// An instance of CalendarView (or simply, a calendar view) is a
/// means for displaying and interacting with a gridstyle layout of date-cells

open class CalendarView: UICollectionView {
    ...
    /// The scroll direction of the sections in Calendar.
    ...
    weak open var calendarDelegate: CalendarViewDelegate? {
        didSet { lastMonthSize = sizesForMonthSection() }
    }
    
    /// The object that acts as the data source of the calendar view.
    weak open var calendarDataSource: CalendarViewDataSource? {
    ...
    let dateGenerator = DateConfigGenerator()
    
    /// Implemented by subclasses to initialize a new object (the receiver) immediately after memory for it has been allocated.
    public init() {
        super.init(frame: .zero, collectionViewLayout: UICollectionViewFlowLayout())
        setupNewLayout(from: collectionViewLayout as! CalendarLayoutProtocol)
    }
    
    /// Initializes and returns a newly allocated collection view object with the specified frame and layout.
    @available(*, unavailable, message: "Please use CalendarView() instead. It manages its own layout.")
